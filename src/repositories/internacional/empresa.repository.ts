...
export class EmpresaRepository extends DefaultCrudRepository<
  ...
  public obtenerEmpresaByRut(rut: any): Promise<any> {
    let query = "select a.id, pj.id persona_juridica_id, pj.identificador, pj.razon_social, pj.nombre_fantasia, te.nombre as tipo_empresa,\n" +
      ...
      "dpn.codigo_comuna as codigo_comuna_rl, dpn.codigo_region as codigo_region_rl, dpn.telefono_fijo as telefono_fijo_rl,dpn.telefono_movil as telefono_movil_rl, dpn.texto as texto_rl, r.nombre, c.nombre\n" +
      "from empresa a\n" +
      ...
      "left join direccion_persona_natural dpn on pn.id = dpn.persona_id\n" +
      "left join region r on r.codigo = dpn.codigo_region\n" +
      "left join comuna c on c.codigo = dpn.codigo_comuna\n" +
